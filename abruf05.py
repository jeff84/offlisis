...
def create_db_structure(conn):
    sql = "DROP TABLE Tabelle05"
    conn.execute(sql)
    conn.commit()
    sql = "DROP TABLE Spiele05"
    conn.execute(sql)
    conn.commit()
    sql = """
    CREATE TABLE Tabelle05 (
    ...
    conn.commit()
    #sql = """
    #CREATE INDEX ix_Tabelle05_Mannschaft ON Tabelle (Mannschaft)
    #"""
    #conn.execute(sql)
    #conn.commit()
...
def main():
    ...
    for row2 in table2.findAll('tr')[1:]:		
        ...
        Erg = True
        Ang = True
        zeile2 = (Spiel, Datum, DatumS, Uhrzeit, Heim, Gast, ToreH, ToreG, Erg, Ang)
